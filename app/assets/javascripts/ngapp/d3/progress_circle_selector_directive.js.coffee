...
.directive 'progressCircleSelector', ['$timeout', ($timeout) ->
  restrict: 'EA'
  scope: {
    student: '=',
    width: '=',
  ...
  link: (scope, element, attrs) ->
    ...
    chart = d3.select(element[0])
      .attr("id", scope.identifier)
      .append('svg')
      .attr("width", width)
      .attr("height", height)
      .attr("viewBox", "0 0 " + width + " " + height)
      .attr("preserveAspectRatio", "xMidYMid")
      .attr("id", "svg" + scope.identifier)
      .append("g")
    ...
    scope.render = (student) ->
      ...
      d3.select("#svg" + scope.identifier + " circle").remove()
      d3.select("#svg" + scope.identifier + " pattern").remove()
      ...
      photoCircle = d3.select(svg)
        ...
        .style("fill", "url(#photo"+scope.identifier+")")

      image = d3.select(svg)
        .append("pattern")
        .attr("id", "photo" + scope.identifier)
        .attr("x", 0)
        .attr("y", 0)
        .attr("width", 1)
        .attr("height", 1)
    ...
    chartSelect = $("#svg"+ scope.identifier)
