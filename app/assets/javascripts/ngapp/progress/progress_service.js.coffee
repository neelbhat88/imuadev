...
.service 'ProgressService', ['$http', '$q', ($http, $q) ->
  ...
  @getUserProgressForModule = (user_id, module_title) ->
    $http.get "api/v1/users/#{user_id}/progress_2?module_title=#{module_title}"

  @getUserProgressForTimeUnitAndModule = (user_id, time_unit_id, module_title) ->
    $http.get "api/v1/users/#{user_id}/progress_2?time_unit_id=#{time_unit_id}&module_title=#{module_title}"

  @getRecalculatedUserProgress = (user_id, time_unit_id, module_title) ->
    $http.get "/api/v1/users/#{user_id}/progress_2?time_unit_id=#{time_unit_id}&module_title=#{module_title}&recalculate=true"
