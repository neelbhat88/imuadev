...
.directive 'studentSelector', ['$location', 'ModuleService', '$modal', ($location, ModuleService, $modal) ->
  restrict: 'E'
  scope: {
    currentuser: '='
  }
  link: (scope, element, attrs) ->
    ...
    scope.$on("studentSelect", (event, student) ->
      scope.studentSelect(student)
    )

    scope.$on("toggleSelectionMode", (event) ->
      scope.toggleSelectionMode()
    )

    scope.$on("selectGroup", (event, args) ->
      scope.selectGroup(args.students, args.groupIsSelected)
    )
